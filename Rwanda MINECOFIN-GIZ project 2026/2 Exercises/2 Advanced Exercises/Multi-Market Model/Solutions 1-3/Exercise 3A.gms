$TITLE Demand-supply multi-market training model (Exercise 3)
...
*** New ***
 d3(C,CP)        cross-price elasticity
;

*** New ***
TABLE MAP(C,CP)  cross-commodity mapping
         agr     ind
 agr      0       1
 ind      1       0
;
...
*i.e., slope of curve multiplied by initial price/quantity ratio
 ...
 d2(C) = 0.75 * (QD0(C)/Y0);
*** New ***
 d3(C,CP)$(MAP(C,CP) EQ 1) = 0.50 * (QD0(C)/P0(CP));

*calculate intercept parameters for demand and supply curves
 s0(C) = QS0(C) - s1(C)*P0(C);
*** New ***
 d0(C) = QD0(C) + d1(C)*P0(C) - d2(C)*Y0 - SUM(CP, d3(C,CP)*P0(CP));
...
*** New ***
 DEMAND(C)..     QD(C)   =E= d0(C) - d1(C)*P(C) + d2(C)*Y + SUM(CP, d3(C,CP)*P(CP));
...
*** New ***
DISPLAY d0, d1, d2, d3, s0, s1, QD0, QD.L, QS0, QS.L, P0, P.L, Y0, Y.L;